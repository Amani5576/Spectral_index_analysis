...
def printflux(filename, rm, region, noise, bgsub, background=0, label='', verbose=False, save_to_file=None):
    ...
    # Create the header for the output text file
    header = "Filename\tLabel\tFrequency(Hz)\tFlux(Jy)\tError(Jy)\tSigma_3(Jy)\tSigma_5(Jy)\n"

    for i in range(rm.nchans):

        freq = rm.frq[i]
        if noise:                                                                              #Included row for 3-sigma and 5-sigma detections
            line = f"{filename}\t{label}\t{freq:8.4g}\t{fg.flux[i]:10.6g}\t{fg.error[i]:10.6g}\t{fg.rms[i]*3:10.6g}\t{fg.rms[i]*5:10.6g}\n"
            print(line, end='')
        else:
            line = f"{filename}\t{label}\t{freq:8.4g}\t{fg.flux[i]:10.6g}\n"
            print(line, end='')

        output_lines.append(line)

    # Write to file if save_to_file is provided
    if save_to_file:
...
def flux_for_files(files, fgr, bgr=None, individual=False, bgsub=False, action=printflux, verbose=False, save_to_file=None):
    ...
    save_to_file -- filename to save the output to (optional)
    """
    ...
    for filename in files:
        ...
        if individual:
            ...
            r = action(filename, rm, fg_ir, noise, bgsub, background, verbose=verbose, save_to_file=save_to_file)
